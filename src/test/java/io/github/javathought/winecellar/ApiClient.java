...
public class ApiClient {
    ...
    /**
     * Call getBottles with empty body. 
    ...
    /**
     * Call addBottle with empty body. 
     * @param handler The handler for the asynchronous request
     */
    public void addBottleWithEmptyBody(
        Handler<AsyncResult<HttpResponse>> handler) {
        // Check required params
        ...
        MultiMap requestCookies = MultiMap.caseInsensitiveMultiMap();
        this.attachRolesSecurity(request, requestCookies);
        this.attachCertSecurity(request, requestCookies);
    ...
    /**
     * Call getBottleByBottleId with empty body. 
     * @param bottleId Parameter bottle_id inside path
     * @param handler The handler for the asynchronous request
     */
    public void getBottleByBottleId(
        Long bottleId,
        Handler<AsyncResult<HttpResponse>> handler) {
        // Check required params
        if (bottleId == null) throw new RuntimeException("Missing parameter bottleId in path");
        

        // Generate the uri
        String uri = "/bottles/{bottle_id}";
        uri = uri.replaceAll("\\{{1}([.;?*+]*([^\\{\\}.;?*+]+)[^\\}]*)\\}{1}", "{$2}"); //Remove * . ; ? from url template
        uri = uri.replace("{bottle_id}", this.renderPathParam("bottle_id", bottleId));
    ...
    /**
     * Call getCellarInformation with empty body. 
     * @param handler The handler for the asynchronous request
     */
    public void getCellarInformation(
        Handler<AsyncResult<HttpResponse>> handler) {
        // Check required params
        

        // Generate the uri
        String uri = "/information";
    ...
    private void attachRolesSecurity (HttpRequest request, MultiMap cookies) {

        this.addHeaderParam("roles", this.Roles_token, request);
        
    }

        private void attachCertSecurity (HttpRequest request, MultiMap cookies) {

        this.addHeaderParam("client-cert", this.Cert_token, request);
    ...
    /**
     * Set access token for security scheme Roles
     */
    public void setRolesToken(String token) {
        this.Roles_token = token;
    }
    
    /**
     * Set access token for security scheme Cert
     */
    public void setCertToken(String token) {
        this.Cert_token = token;
    }
